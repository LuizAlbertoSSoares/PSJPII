...
while (!feof($arquivo)){
	...
	if (substr($linha,8-1,1) === "3" and substr($linha,14-1,1) === $BancoCliente ) {
		...
		$NossoNumero = substr($linha,39-1,8);
		$Campanha = "4"; 
		$Parcela = substr($linha,45-1,2);
		$NumeroDocumento = $NossoNumero;
		$ValorDoDesconto = substr($linha, 189-1,25) / 100;
		$Quota = substr($linha,40-1,7);
...
//***** INICIO BAIXAR TITULO PAGO
		if ($controle != null){
			...
			$TIT_VALOR_RECEBIDO = $valor;
		  	...
		  	."  WHERE TIT_NOSSO_NUMERO = ".$NossoNumero;
